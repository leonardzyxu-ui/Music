...
- Native AppKit traffic-light buttons are present, visible, and enabled according to the protected `window-controls` diagnostic. Latest diagnostic places them at `x=34/57/80, y=5` after correcting the previous too-high/transparent-titlebar placement.
- The AppKit frame view and content view now both use the same continuous 38-point corner mask, with a 28-point inner sidebar corner after the 10-point sidebar inset. The bridge contract now fails if this regresses below an Apple Music-like radius or if the masks/clear window background are lost.
- Added `script/audit_window_corners.py`, a Music-window-only screenshot audit that saves `screenshots/window-corner-audit.png` plus a top-left crop and fails if the visible outer corner becomes too tight or too huge again.
...
- The outer window now favors real macOS hit-testing/resizing plus AppKit frame/content rounded masks over the earlier fragile SwiftUI-only fake shell. Leo still needs to visually approve the exact corner radius and traffic-light position in the live app.
